module PatientsHelper  
  def add_measurement(session, form_builder)
    ...
    form_builder.fields_for :measurements, session.measurements.build, :child_index => 'NEW_RECORD' do |measurement_form|
      html = render(:partial => 'measurement', :locals => { :f => measurement_form })
      link_to_function 'Add Measurement', 
                       "$('#{escape_javascript(html)}'.replace(/NEW_RECORD/g, $('.measurement').length)).insertBefore('#add_measurement')",
                       :id  => "add_measurement"
    end
  end
end